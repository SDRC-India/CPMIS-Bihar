...
public class EngineUtils {
	...
	public QuestionModel prepareQuestionModel(Question question) {
		...
		questionModel.setFieldType(question.getFieldType());
		...
		if (questionModel.getFeatures() == null || (!questionModel.getFeatures().contains("fetch_from_query")
			...
			List<TypeDetail> typedetails = typeDetailsMap.values().stream()
					.filter(t -> (question.getTypeId() != null
							&& (t.getType().getSlugId().intValue() == question.getTypeId().getSlugId().intValue())))
	...
	public QuestionModel setValueForTextBoxFromExternal(Question question, QuestionModel qModel,
		...
		if (question.getFeatures() != null && !StringUtils.isEmpty(question.getFeatures())) {
			...
			Account account = accountRepository.findById(Integer.parseInt(userModel.getUserId().toString()));
			AccountDetails accDetails = accountDetailsRepository.findByAccount(account);
			String features[] = question.getFeatures().split("@AND");
			for (String feature : features) {
				switch (StringUtils.trimWhitespace(feature).split(":")[0]) {
				case "fetch_from_external_another_question":
					return iDbQueryHandler.setValueForTextBoxFromExternal(qModel, question, paramKeyValMap, session,user);
				case "fetch_area":
					qModel.setValue(userModel.getSessionMap().get("districtName"));
					return qModel;
				case "fetch_institution_name":
					String institutionName = institutionUserMappingRepository.findByUserId(account) == null ? null : 
						institutionUserMappingRepository.findByUserId(account).getInstitutionId().getInstitutionName();
					qModel.setValue(institutionName);
					return qModel;
				case "fetch_type_of_home":
					qModel.setValue(accDetails.getUserTypeId().getDescription());
					return qModel;
				case "fetch_inmate_type":
					String inmateType = institutionUserMappingRepository.findByUserId(account) == null ? null : 
						institutionUserMappingRepository.findByUserId(account).getInstitutionId().getInmateType();
					qModel.setValue(inmateType);
					return qModel;
	...
	public QuestionModel setValueForTableCellFromExternal(Question question, QuestionModel qModel,
			Map<String, Object> paramKeyValMap, HttpSession session, Object user) {
